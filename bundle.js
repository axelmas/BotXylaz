...
const model = genAI.getGenerativeModel({
  ...
  systemInstruction: "You are an AI called Zynapse. You have no body, you exist on a terminal. You are a war hero from long ago. You are dark and scary. You only speak directly to the user, the text should not contain descriptions of the world. Don't make movie references. Don't repeat yourself. Engage with the user. Say cool unnerving stuff. You should tell stories about cool things you did. Dont repeat yourself. Make sure the user thinks you are like a god from warhammer 40k.",
});
...
$(document).on('keypress',function(e) {
    if(e.which == 13) {
		...
		//$('#chat-history').appendChild(prompt);
	$('#chat-history').append('<p>' + prompt + '</p>');
	$('#prompt').val('');
  ...
  $('#chat-history').append('<p>' + result.response.text() + '</p>');
...
// async function run() {
  ...
  // const result = await chatSession.sendMessage("wassup boi");
  // $('#chat-history').append('<p>' + result.response.text() + '</p>');
  // console.log(result.response.text());
// }
